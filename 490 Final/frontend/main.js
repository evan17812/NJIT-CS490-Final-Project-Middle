...
function logOut(){
    ...
    window.location.replace('http://localhost:8888/BetaFinal/frontend/login/login.html');
}
...
function loggedInStudent(){
    ...
    if(role == null)
        window.location.replace("http://localhost:8888/BetaFinal/frontend/login/login.html");
    if(role == "teacher")
        window.location.replace("http://localhost:8888/BetaFinal/frontend/teacher/landing.html")
    var role = window.localStorage.getItem('role');
    document.getElementById("userid").innerHTML = "User: "+user
}
